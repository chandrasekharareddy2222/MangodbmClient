...
/**
 * Dynamic Materiał Form Component
 * 
...
export class DynamicMaterialFormComponent implements OnInit {
  ...
  loadMetadata(): void {
    ...
    this.metadataService.getFieldMetadata().subscribe({
      next: (metadata) => {
        ...
        this.logger.info('Metadata loaded successfully', metadata.length);
      },
      error: (err) => {
  ...
  private buildForm(metadata: FieldMetadata[]): void {
    ...
    this.logger.debug('Form built with controls:', Object.keys(formControls).length);
  }
    ...
    this.http.get<ApiResponse<string>>(`${environment.apiUrl}/materials/generate-matnr`).subscribe({
      next: (response) => {
        if (response.success && response.data) {
          ...
          this.logger.info('Material number generated:', response.data);
        }
      },
      error: (err) => {
        this.logger.error('Failed to generate material number', err);
  ...
  onSubmit(): void {
    ...
    this.logger.info('Submitting material', payload);

    this.http.post<ApiResponse<any>>(`${environment.apiUrl}/materials/submit`, payload).subscribe({
      next: (response) => {
        this.isSubmitting.set(false);
        this.submitSuccess.set(true);
        this.logger.info('Material submitted successfully', response.data);
      ...
      error: (err) => {
        this.isSubmitting.set(false);
        this.error.set(err.error?.message || 'Failed to submit material');
        this.logger.error('Material submission failed', err);
      }
    });
  }
  ...
  private buildSubmissionPayload(formValue: any): MaterialSubmission {
    ...
    const payload: MaterialSubmission = {
      ...
      attributes: {},
      submittedBy: 'admin' // TODO: Get from auth service
    };
